...
*Имя класса должно совпадать с ID модуля, вместо точки между именем парнера и именем модуля знак
подчеркивания
*Класс модуля должен быть наследником системного класса модулей Битрикс CModule*/
Class library_read extends CModule {
    /*Обязательные свойства объекта модуля*/
    const MODULE_ID = "library.read"; var $MODULE_ID = "library.read"; var $MODULE_VERSION;
    ...
    function  construct()
    {
        ...
        $this->MODULE_NAME = GetMessage("library.read_MODULE_NAME");
        $this->MODULE_DESCRIPTION = GetMessage("library.read_MODULE_DESC");
        $this->PARTNER_NAME = GetMessage("library.read_PARTNER_NAME");
        $this->PARTNER_URI = GetMessage("library.read_PARTNER_URI");
    }
    ...
    function InstallEvents()
    {
        ...
        $em->registerEventHandler('sale', 'OnBasketAdd', self::MODULE_ID,
        '\CompanyNamespace\Promotions\Connector', 'OnBasketAdd');
        $em->registerEventHandler('sale', 'OnBasketUpdate', self::MODULE_ID,
        '\CompanyNamespace\Promotions\Connector', 'OnBasketUpdate');
        */
        return true;
    }
    function UnInstallEvents()
    {
        /* Пример
        $em = \Bitrix\Main\EventManager::getInstance();
        $em->unRegisterEventHandler('sale', 'OnBasketAdd', self::MODULE_ID,
        '\CompanyNamespace\Promotions\Connector', 'OnBasketAdd');
        $em->unRegisterEventHandler('sale', 'OnBasketUpdate', self::MODULE_ID,
        '\CompanyNamespace\Promotions\Connector', 'OnBasketUpdate');
        */
        return true;
    }
    ...
    function InstallFiles($arParams = array())
    {
            ...
            . self::MODULE_ID . "/admin"))
        {
            if ($dir = opendir($p))
            {
                ...
                file_put_contents($file =$_SERVER["DOCUMENT_ROOT"] . "/bitrix/admin/"
    . self::MODULE_ID . "_"
...
/*ОБЯЗАТЕЛЬНЫЙ ДЛЯ РЕАЛИЗАЦИИ МЕТОД*/
        ...
        {
            global $DB, $DBType;
            $DB->RunSQLBatch($_SERVER["DOCUMENT_ROOT"]."/local/modules/"
                .self::MODULE_ID."/install/db/".strtolower($DBType)."/install.sql");
return true;
        }
    }
